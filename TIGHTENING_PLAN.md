...
30. **Surface-framing rotation on §2 parenthetical "prohibition"** — Validators periodically claim that 1.04 §6.4 or §6.11 prohibits parenthetical titles in §2 Out of Scope routing pointers. The claim misreads both sections: §6.4 specifies the routing-pointer minimum format ending at "Policy [X.XX]"; §6.11 says the first-use-parenthetical convention "does not apply to" Scope section routing pointers — meaning Scope use doesn't satisfy the first-use obligation, NOT that Scope prohibits parentheticals. 1.04 v1.4's OWN §2 Out of Scope uses parentheticals (`Work order records: Policy 3.05 (Service Request Fulfillment Policy).`). Gemini's strict-prohibition reading would invalidate the constitution itself. Reflexive decline on sight; cite 1.04 §2 demonstrating the permissive pattern. Third-recurrence rule triggered: when a validator raises this across three or more rounds with rotated surface framings, reflexive decline without re-engaging the specific framing. (2.04 Phase 5B entry 28, Rounds 1 and 3, Gemini — Round 1 raised and declined; Round 2 quiet; Round 3 re-raised with expanded "strict format mandate" framing, declined again. Meta-pattern matches decline #23 mechanics on a different architectural target.)
...
**Last refreshed:** April 20, 2026 (2.04 Phase 5B entry 28 ✅ complete — v2.3 committed after 3 validator rounds; tickets #69, #70 closed; ripple filed #357 (2.02 §4 Sanctions NRS 205.4765 criminal-only correction); sweep absorption notes on #208/#235/#245/#264/#291/#6; new standing decline #30 added (surface-framing rotation on §2 parenthetical "prohibition" — Gemini Rounds 1 and 3, third-recurrence rule triggered). 2.04 open ticket count 2→0. Open ticket total: 94 (was 95; -#69, -#70, +#357 = net -1). §5 row moved D→T.) **Previously refreshed:** April 19, 2026 (95-ticket audit against compiled PDF ✅ — verified every remaining open ticket describes a real still-present defect or valid forward-scheduled action. #183 closed (5.02 old 5.07 title sweep confirmed complete manual-wide; 5.02 row count 5→4). #334 partial pre-satisfaction noted (1 of 4 sub-asks — open-ended-exception bar — already landed in 5.01 §6.6 L384; 3 asks remain; ticket left open without comment per IT Director direction). Duplicate scan across 38 GLOBAL-labeled tickets confirmed no additional #7/#10-class duplicates; #264 and #291 are legitimate parent-child hierarchies with documented scope beyond their children. Methodological finding: pdftotext hyphenation at line breaks produces false negatives — hyphen-joined text analysis required for reliable defect verification. Preceded by full plan-vs-GitHub reconciliation ✅ — API-driven ground-truth audit against 97 open issues surfaced drift across 10 §5 rows and 2 untracked globals. All drift was undercount (no phantom closed tickets). Corrections applied: 8 §5 rows gained missing tickets (2.03 +#346/+#351, 2.05 +#333, 2.09 +#326, 2.12 +#330, 2.14 +#332, 5.01 +#334, 5.06 +#331, 6.01 +#352); duplicate closure of #7 (dup of #99) and #10 (dup of #71) eliminated historical parent/child double-tracking on single-target GLOBAL-D work; #353 (Phase 10 header normalization) and #355 (Phase 9 LRDA sweep) added to §3 Architectural Roots as phase-deferred entries. Post-reconciliation coverage: 95 open tickets = 86 in §5 per-policy rows + 9 §3 roots. Convention normalized: all single-target GLOBAL-D children counted in §5 (previous inconsistency where #292/#293/#271 were counted but #7/#10 parents were not now resolved by closing the duplicates). Preceded by Bucket 1 ripple sweep expanded: 3.13 v2.2 (#322), 4.03 v2.1 (#323), 3.08 v2.2 (#325 with expanded scope), 3.01 v2.4 (#327). Preceded by 2.02 Phase 5B entry 27 ✅ complete — v2.4 committed after 3 validator rounds + IT Director §3 label challenge; ripples filed #354 (1.07 remove 5-year CJI rescreening, Path A), #355 (manual-wide LRDA verification sweep), #356 (2.01 §3 label normalization); sweep absorption notes on #208/#235/#245/#264/#6/#7/#8/#10; new standing decline #29 added (cross-validator pushback on locked operational-reality decisions via cross-policy dependency citation, third-recurrence rule).
...
| 2.02 | Personnel Security | T+ | v2.4, April 19 | 1 | #357 |
...
| 28 | ~~2.04 (Access Control)~~ — ✅ v2.3, April 20, 2026. Tickets #69, #70 closed. Ripple filed: #357 (2.02 v2.4 §4 Sanctions — NRS 205.4765 civil/criminal overstatement; statute is criminal-only). Sweep absorption notes on #208, #235, #245, #264, #291, #6. Architectural landings: §2 rebuilt with In/Out of Scope colon-format routing pointers (2.13, 2.05, 2.07, 2.10, 2.11, 2.02, 2.14, 5.01 §6.6); §3 restructured to 6 roles descending authority (IT Director → County Manager → AID → Department Heads → HR → All County Personnel) matching Ch2 LASO pattern, with "IT Administrators" and "Access Control Authority" invented constructs retired and "Department Heads / Data Owners" double-label collapsed; §4 rebuilt to Assessment/Enforcement/Exceptions flat bolded-label structure with Universal Applicability + Sanctions optional governed labels + Constraint sub-bullet on CJI exception routing; §5 split into Cited References (every entry body-cited per 1.04 §6.7 strict enforcement) and Framework Alignment with CJIS IA-5(2)(c) invalid citation replaced by verified IA-11 (Re-Authentication); §6.1 Termination/Transfer split into Involuntary Termination (30 min) + Standard Termination (24 hr, CJIS PS-4) + Transfer (24 hr, modify not disable, CJIS PS-5) + High-Risk (30 min HR-notification OR IT Director independent discovery per IT Director ruling) + Expired/Unauthorized (7 CD) + Inactivity (30 CD stricter-than-CJIS-90-day with Rationale tag); §6.2 separation-of-duties collapsed to by-reference pointer at 1.08 §6.6; §6.6 device-lock 15-min stricter-than-CJIS-AC-11-30-min annotated with Rationale tag, AC-11 NOTE exemption restructured as Exception sub-bullet; §6.7 FIPS 140 inline replaced with CMVP by-reference to 1.11 §6.5 (#291 absorption); §6.7 CJIS Advanced Authentication (v5.x legacy term) replaced with CJIS v6.0 IA-2(1)/(2); §6.9 retention points to 1.11 §6.9 (inherits LRDA 20071728 "IT Access Control Reviews and Audit Trails"); §6.9 Documentation atomic split into three sub-bullets; §6.1 Transfer boundary sentence restructured as Clarification sub-bullet; High-Risk Individual definition sourced from 2.02 §6.7; NRS 205.4765 Sanctions corrected from civil/criminal to criminal-only. Three cross-validator rounds; standing declines #1, #7, #12, #20, #22, #26 held repeatedly; new Pattern #30 promoted to §4 (surface-framing rotation on §2 parenthetical prohibition — Gemini Rounds 1 and 3, third-recurrence rule triggered). 2.04 Round 2 saw ChatGPT triple-hit pattern #7 (three simultaneous stale-source findings on §3, §4, §5 structure already tightened in v2.1). | 2 (#69, #70) | Foundational — 2.05 I&A depends on it |
